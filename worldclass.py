...
class localMinecraftWorld:
    def __init__(self, name):
        ...
        retcode = subprocess.call(r'mcpetool.exe leveldat get --path C:\Users\david\AppData\Local\Packages\Microsoft.MinecraftUWP_8wekyb3d8bbwe\LocalState\games\com.mojang\minecraftWorlds\ZwNtX6xyAQA=', stdout=FNULL, stderr=subprocess.STDOUT)
        FNULL.close()
    ...
    def verifyPlayerMetadata(self):
        ...
        if self.player_metadata and len(self.player_metadata) == 3:
            self.full_id = True
    ...
    def moveLocalPlayerToNewRemotePlayer(self):
        with open(self.script_path + r'\\myid.json', 'w') as your_id:
            ...
            self.player_metadata_id['nbt'][0]['value'][-1]['value'] = player_key
    ...
    def localToCloudSetup(self):
        ...
        if self.verifyPlayerMetadata():
            print('verifies player metadata')
    ...
    def cloudToLocalSetup(self, cloud_date, offset): # ========FIX======== u dumm dumm
        ...
        timestamp = datetime(int(date_separated[0][0]), int(date_separated[0][1]), int(date_separated[0][2]), int(date_separated[1][0]), int(date_separated[1][1]), int(date_separated[2][0]), int(date_separated[2][1])).timestamp() - offset
        os.utime(self.path, (timestamp, timestamp))

        if self.verifyPlayerMetadata() and self.verifyPlayerExistance():
            self.moveRemotePlayerToLocalPlayer()
        else:
            self.addNewLocalPlayer()

    def uploadWorldZip(self, drive, id, parent_id):
